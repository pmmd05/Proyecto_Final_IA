...
def convertir_a_log_mel_spectrogram(audio):
    ...
    # Para CNN 2D necesitamos canal:
    # tiempo x bandas_mel x canal
...
def crear_modelo_cnn_mel(input_shape, numero_clases):
...
def guardar_grafica_entrenamiento(historial, ruta_salida):
    ...
    plt.title("Exactitud durante entrenamiento - CNN Mel-Spectrogram")
    plt.xlabel("Época")
    plt.ylabel("Accuracy")
    plt.legend()
    plt.grid(True)
...
def guardar_grafica_perdida(historial, ruta_salida):
    ...
    plt.title("Pérdida durante entrenamiento - CNN Mel-Spectrogram")
    plt.xlabel("Época")
    plt.ylabel("Loss")
    plt.legend()
    plt.grid(True)
...
def evaluar_modelo(modelo, dataset_prueba, nombres_clases):
    ...
    print("\n=== REPORTE DE CLASIFICACIÓN - CNN MEL ===\n")
    ...
    plt.title("Matriz de confusión - CNN Mel-Spectrogram")
    plt.savefig(
        MODELS_DIR / "confusion_matrix_cnn_mel.png",
        dpi=160,
        bbox_inches="tight"
    )
    plt.close()
...
def main():
    ...
    print("\n=== ENTRENAMIENTO CNN 2D + MEL-SPECTROGRAM ===\n")
    ...
    for muestras, _ in dataset_train.take(1):
        input_shape = muestras.shape[1:]
        break

    print(f"\nForma de entrada CNN Mel: {input_shape}")
    print("Interpretación: tiempo x bandas Mel x canal")

    modelo = crear_modelo_cnn_mel(
    ...
    callbacks = [
        ...
        tf.keras.callbacks.ModelCheckpoint(
            filepath=str(MODELS_DIR / "domotica_cnn_mel.keras"),
    ...
    modelo.save(MODELS_DIR / "domotica_cnn_mel.keras")

    with open(MODELS_DIR / "class_names_cnn_mel.json", "w", encoding="utf-8") as archivo_json:
        json.dump(
            nombres_clases,
            archivo_json,
            indent=2,
    ...
    guardar_grafica_entrenamiento(
        historial,
        MODELS_DIR / "training_accuracy_cnn_mel.png"
    )

    guardar_grafica_perdida(
        historial,
        MODELS_DIR / "training_loss_cnn_mel.png"
    )
    ...
    print("\nEntrenamiento CNN Mel terminado.")
    print("\nArchivos generados:")
    print(f"- {MODELS_DIR / 'domotica_cnn_mel.keras'}")
    print(f"- {MODELS_DIR / 'class_names_cnn_mel.json'}")
    print(f"- {MODELS_DIR / 'training_accuracy_cnn_mel.png'}")
    print(f"- {MODELS_DIR / 'training_loss_cnn_mel.png'}")
    print(f"- {MODELS_DIR / 'confusion_matrix_cnn_mel.png'}")
